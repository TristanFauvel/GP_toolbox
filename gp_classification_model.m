classdef gp_classification_model < gpmodel
    ...
    methods
        function model = gp_classification_model(D, meanfun, kernelfun, regularization, hyps, lb, ub, type, link, modeltype, kernelname, ns)
            ...
            model = model@gpmodel(D, meanfun, kernelfun, regularization, hyps, lb, ub, kernelname, ns);
            model.link = link;
        ...
        function [sample_normalized, sample, fmax] = sample_max_GP(approximation, xtrain_norm, ctrain, theta, model, post, optim)
            ...
            if strcmp(optim.task, 'max')
                f = sample_g;
                dfdx = dsample_g_dx;
                % sample_normalized = multistart_minConf(@(x)takemax(x,f, dfdx, model.sdims), model.lb_norm, model.ub_norm, ncandidates,init_guess, options, 'objective', 'max', 'dims', model.xdims);

                sample_normalized = multistart_minConf(@(x) deriv(x, f, dfdx), model.lb_norm, model.ub_norm, ncandidates,init_guess, options, 'objective', 'max', 'dims', model.xdims);
                fmax = f(sample_normalized);
