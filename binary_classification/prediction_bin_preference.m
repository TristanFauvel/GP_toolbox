...
if ~isempty(xtest)
    if nargout>4
        [ks, ~, dks_dx] = kernelfun(theta, xtest, xtest, false, regularization);  %%%% Careful : normally here it's 'false', but I put 'true'in order to get a correct computation of the derivative, only ok if noise = 0
        diag_ks= diag(ks);
        n=size(xtest,2);
        ...
        try ks = kernelfun(theta, xtest, xtest, false, regularization);
            diag_ks= diag(ks);
        catch
            diag_ks = zeros(ntst,1);
            for l = 1:ntst
                diag_ks(l) = kernelfun(theta, xtest(:,l), xtest(:,l), false, regularization);
            end
        end
    end
end
