function [C, dC, dC_dx] = product_kernel(theta, x0, x, training, kernelfun1, kernelfun2, d1, d2, dtheta1, dtheta2)
DEFAULT('x', x0);
x01 = x0(d1,:);
x02 = x0(d2,:);
x1 = x(d1,:);
...
if nargout == 1
    C1 = kernelfun1(theta1, x01, x1, training);
    C2 = kernelfun2(theta2, x02, x2, training);
elseif nargout == 2
    [C1, dC1] = kernelfun1(theta1, x01, x1, training);
    [C2, dC2] = kernelfun2(theta2, x02, x2, training);  
...
elseif nargout == 3
    [C1, dC1, dC1_dx] = kernelfun1(theta1, x01, x1, training);
    [C2, dC2, dC2_dx] = kernelfun2(theta2, x02, x2, training);    
    dC =  NaN(ntr, ntst, numel(theta));
    dC(:,:,dtheta1) = dC1.*C2;
    dC(:,:,dtheta2) = dC2.*C1; %note: here we assume that the hyperparameters are independent between the two kernels, which may not always be the case
